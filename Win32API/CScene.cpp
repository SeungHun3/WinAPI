...
void CScene::render(HDC _dc)
{
	for (UINT i = 0; i < (UINT)GROUP_TYPE::END; i++)
	{
		if ((UINT)GROUP_TYPE::TILE == i) // 타일 dead없다는 가정하에
		{
			render_tile(_dc);
			continue;
		}
...
void CScene::CreateTile(UINT _iXCount, UINT _YCount)
{
...
void CScene::LoadTile(const wstring& _strRelativePath)
{
	...
	// FILE = 커널 오브젝트
	FILE* pFile = nullptr;
	_wfopen_s(&pFile, strFilePath.c_str(), L"rb"); // 모드: w 쓰기, r 읽기
	assert(pFile);
